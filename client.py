...
app.secret_key = "dein_super_geheimer_und_einzigartiger_schluessel_hier_aendern_DRINGEND_PYTHONANYWHERE_CLIENT"
...
def show_termux_notification(title, content, notification_id=None):
    ...
    try:
        ...
        print(f"CLIENT NOTIFICATION TIMEOUT: Befehl termux-notification dauerte zu lange.")
    ...
    if server_socket_global and socket_is_currently_connected:
        try:
            ...
            with client_data_lock:
                ...
                client_view_data["error_message"] = f"Verbindung zum Server verloren. Aktion '{action_sent}' nicht gesendet."
        except Exception as e:
            print(f"CLIENT SEND (UNEXPECTED ERROR): Senden von '{action_sent}' fehlgeschlagen: {e}.")
            traceback.print_exc()
            with client_data_lock:
                client_view_data["is_socket_connected_to_server"] = False
                client_view_data["error_message"] = f"Unerwarteter Fehler beim Senden von '{action_sent}'."
    else:
    ...
    while True:
        ...
        if not user_wants_to_connect:
            with client_data_lock:
                if client_view_data["is_socket_connected_to_server"]:
                    client_view_data["is_socket_connected_to_server"] = False
                if client_view_data.get("game_state",{}).get("status") != "disconnected": # Nur wenn nicht schon disconnected ist
            ...
            try:
                ...
                with client_data_lock:
                    ...
                    if client_view_data.get("player_id") and client_view_data.get("player_name"):
                        ...
                        print("CLIENT NET: Keine Spieler-ID/Name für Rejoin vorhanden.")
                ...
                with client_data_lock: client_view_data["game_state"]["status_display"] = f"Verbindung zu {current_host_to_connect}:{current_port_to_connect} fehlgeschlagen: Server nicht erreichbar."
                time.sleep(3); continue
        ...
        try:
            if not server_socket_global:
                print("CLIENT NET (RECEIVE ERROR): server_socket_global ist None trotz is_socket_connected_to_server=True.")
            ...
            while '\n' in buffer:
                ...
                with client_data_lock:
                    ...
                    if msg_type == "game_update":
                        ...
                        # Alte Fehlermeldungen löschen, die sich auf Verbindungsherstellung/Join bezogen, wenn Update erfolgreich
                        if client_view_data.get("player_id"):
                            if client_view_data.get("error_message","").startswith("Verbindung") or \
                               client_view_data.get("error_message","").startswith("Senden der Join-Anfrage") or \
                               client_view_data.get("error_message","").startswith("Nicht mit Server verbunden"):
                                client_view_data["error_message"] = None

                        if client_view_data.get("player_id") and \
                           client_view_data.get("offline_action_queue") and \
                           not client_view_data.get("is_processing_offline_queue"):
                            print("CLIENT NET (game_update): Starte Verarbeitung der Offline-Queue.")
                            threading.Thread(target=process_offline_queue, daemon=True).start()
                        ...
                        if event_name == "hider_location_update_due":
                            ...
                            client_view_data["hider_location_update_imminent"] = True # Wird serverseitig in game_update gesendet
                        ...
                        critical_errors_keywords = [
                            "Spiel läuft bereits", "Spiel voll", "Nicht authentifiziert",
                            "bitte neu beitreten", "nicht mehr teil des aktuellen spiels", # kleingeschrieben für robustere Prüfung
                            "server wurde von", "rejoin fehlgeschlagen", "name", "bereits vergeben",
                            "sitzung ungültig"
                        ]
                        error_text_lower = error_text.lower()
                        if any(crit_err in error_text_lower for crit_err in critical_errors_keywords):
                        ...
                        # Lösche spezifische Fehlermeldungen, wenn eine Aktion bestätigt wurde
                        if client_view_data.get("error_message","").startswith("Aktion") or \
                           client_view_data.get("error_message","").startswith("Offline erledigte Aufgabe"):
                           client_view_data["error_message"] = None
...
def get_status():
    with client_data_lock:
        # Sicherstellen, dass die globalen Python-Werte für Host/Port in client_view_data reflektiert werden
...
def connect_to_server_route():
    ...
    with client_data_lock:
        ...
        client_view_data.update({
            ...
            "game_state": { # Reset game_state
                "status": "connecting", 
                "status_display": "Verbinde mit " + server_address + "...",
    ...
    if server_socket_global:
        print(f"CLIENT FLASK: Server details changed or explicit connect, shutting down old socket: {server_socket_global}")
        try:
            # server_socket_global.shutdown(socket.SHUT_RDWR) # Kann Fehler werfen, wenn schon zu
...
def register_player_details_route():
    ...
    # Wichtig: player_name nur im client_view_data setzen, wenn der JOIN_GAME Befehl erfolgreich gesendet wird.
    # Der Server wird den Namen per game_update bestätigen.
    ...
    if socket_conn_ok:
        if not send_message_to_server({"action": "JOIN_GAME", "name": nickname, "role_preference": role_choice}):
            # send_message_to_server setzt client_view_data["join_error"] oder "error_message" bei Fehler
            # Diese wird dann von der UI im nächsten /status Poll abgeholt.
             with client_data_lock:
                if not client_view_data.get("join_error") and not client_view_data.get("error_message"):
                    client_view_data["join_error"] = "Senden der Join-Anfrage fehlgeschlagen (unbekannter Grund)."

    else:
        with client_data_lock: client_view_data["join_error"] = "Nicht mit Server verbunden. Bitte zuerst verbinden."

    # Antwort enthält den aktuellen client_view_data, damit die UI Join-Fehler etc. sieht
    with client_data_lock:
        response_data = client_view_data.copy()
        response_data["session_nickname"] = session.get("nickname")
        response_data["session_role_choice"] = session.get("role_choice")
...
def update_location_from_browser():
    ...
    response_payload = {"success": False, "message": "Standort nicht verarbeitet."} # Default
    if player_id_local and game_can_receive_loc and socket_ok_local:
        ...
        if send_success:
            ...
            response_payload = {"success": True, "message": "Standort an Server gesendet."}
        else:
            # send_message_to_server sets error in client_view_data
            response_payload = {"success": False, "message": "Senden an Server fehlgeschlagen. Standort nicht aktualisiert."}
            # Status 500 ist nicht ideal, da es ein Client-Backend-Fehler ist, nicht unbedingt ein Server-Fehler
            return jsonify(response_payload), 200 # OK, aber success:false
    elif not player_id_local: response_payload = {"success":False, "message":"Keine Spieler-ID bekannt. Bitte zuerst beitreten."}
    elif not game_can_receive_loc: response_payload = {"success":False, "message":f"Spielstatus '{game_status_local}' erlaubt keine Standortupdates."}
    else: response_payload = {"success": False, "message": "Keine aktive Socket-Verbindung zum Spielserver."}
    
    return jsonify(response_payload) # HTTP 200, aber success-Flag in JSON zeigt Ergebnis
...
def handle_generic_action(action_name, payload_key=None, payload_value_from_request=None, requires_player_id=True):
    ...
    player_id_for_action = None
    response_for_ui = {"action_send_success": False} # Default response
    ...
    if requires_player_id:
        ...
        if not player_id_for_action:
            response_for_ui["error_message"] = f"Aktion '{action_name}' nicht möglich (keine Spieler-ID)."
            return jsonify(response_for_ui), 200 # HTTP 200, aber Fehler in JSON

    if payload_key: 
        req_data = request.get_json()
        if req_data is None and payload_key != "ready_status" and payload_key != "force_server_reset_from_ui": # Ausnahmen für bool und parameterlose Aktionen
             response_for_ui["error_message"] = "Keine JSON-Daten im Request für Aktion mit Payload."
             return jsonify(response_for_ui), 200

        val_from_req = req_data.get(payload_value_from_request or payload_key) if req_data else None

        if payload_key == "ready_status":
             if not isinstance(val_from_req, bool):
                 response_for_ui["error_message"] = "Ungültiger Wert für ready_status (muss boolean sein)."
                 return jsonify(response_for_ui), 200
        elif val_from_req is None and payload_key != "force_server_reset_from_ui": # force_server_reset_from_ui braucht keinen expliziten Wert
            response_for_ui["error_message"] = f"Fehlender Wert für '{payload_key}' in Request-Daten."
            return jsonify(response_for_ui), 200
    ...
    response_for_ui["action_send_success"] = success_sent
    
    with client_data_lock: # UI-Nachrichten basierend auf Sendeerfolg
        if success_sent:
            # Alte Fehler löschen, die sich auf Senden bezogen, falls jetzt erfolgreich.
            # Der Server könnte immer noch einen Fehler für die Aktion selbst senden, der dann über game_update kommt.
            if client_view_data.get("error_message", "").startswith("Nicht mit Server verbunden") or \
               client_view_data.get("error_message", "").startswith("Senden an Server fehlgeschlagen"):
                client_view_data["error_message"] = None # Dieser Fehler ist behoben durch erfolgreiches Senden
            response_for_ui["game_message"] = f"Aktion '{action_name}' erfolgreich an Server gesendet."
            # client_view_data["game_message"] = f"Aktion '{action_name}' gesendet." # Wird über Server-Ack oder /status aktualisiert
        else:
            # send_message_to_server hat bereits client_view_data["error_message"] gesetzt
            response_for_ui["error_message"] = client_view_data.get("error_message", 
                                                                    f"Aktion '{action_name}' konnte nicht an den Server gesendet werden (Verbindungsfehler).")
    return jsonify(response_for_ui)
...
def complete_task_route():
    ...
    if not player_id_local or not is_hider_active:
        return jsonify({"action_send_success": False, "error_message": "Aktion nicht möglich (kein aktiver Hider oder keine Spieler-ID)."}), 200

    if not current_task_local or not current_task_local.get("id"):
        return jsonify({"action_send_success": False, "error_message": "Keine aktive Aufgabe zum Erledigen vorhanden."}), 200
    ...
    if socket_ok_local: 
        ...
        response_payload = {"action_send_success": success_sent}
        if success_sent:
            response_payload["game_message"] = f"Anfrage 'Aufgabe erledigt' für '{task_description_for_ui_msg}' an Server gesendet."
        else: # send_message_to_server setzt client_view_data.error_message
            with client_data_lock:
                response_payload["error_message"] = client_view_data.get("error_message", "Senden der Aktion 'Aufgabe erledigt' fehlgeschlagen.")
        ...
        with client_data_lock:
            ...
            client_view_data["current_task"] = None # Optimistisch UI updaten für Offline
            # Die Antwort enthält hier den vollen Client-Status, da die UI sich sofort anpassen soll.
            response_data = client_view_data.copy()
            response_data["action_send_success"] = True # Die Aktion wurde erfolgreich in die Queue gelegt
            response_data["session_nickname"] = session.get("nickname")
            response_data["session_role_choice"] = session.get("role_choice")
...
def force_server_reset_route():
    ...
    if not data or 'server_address' not in data:
        return jsonify({"action_send_success": False, "error_message": "Server-Adresse für Reset fehlt."}), 200
    server_address = data['server_address'].strip()
    if not server_address:
        return jsonify({"action_send_success": False, "error_message": "Server-Adresse darf nicht leer sein."}), 200
    ...
    try:
        ...
        return jsonify({"action_send_success": False, "error_message": "Ungültiger Port in der Adresse."}), 200
    ...
    with client_data_lock:
        ...
        client_view_data["player_is_ready"] = False
        client_view_data["join_error"] = None 
        client_view_data["error_message"] = None # Alte Fehler löschen

        # Offline-Queue für diesen speziellen Fall leeren und Reset-Befehl hinzufügen
        reset_action = {
            "action_for_server": {"action": "FORCE_SERVER_RESET_FROM_CLIENT"},
            "ui_message_on_cache": f"Reset-Befehl für {server_address} in Warteschlange. Verbinde..."
        }
        client_view_data["offline_action_queue"].clear() 
        client_view_data["offline_action_queue"].append(reset_action)
        client_view_data["game_message"] = reset_action["ui_message_on_cache"]
        client_view_data["game_state"]["status_display"] = "Sende Reset-Befehl..." # UI Feedback
        print("CLIENT FLASK (Reset): Reset-Aktion zur Offline-Queue hinzugefügt.")

    if server_socket_global:
        print(f"CLIENT FLASK (Reset): Shutting down old socket: {server_socket_global}")
        try:
    ...
    # Der Netzwerk-Thread wird die Verbindung (neu) aufbauen und dann die Offline-Queue (mit dem Reset-Befehl) verarbeiten.
    # Die Antwort an die UI ist hier minimalistisch, der eigentliche Zustand kommt über /status.
    return jsonify({"action_send_success": True, "game_message": f"Reset-Anfrage für {server_address} wird vorbereitet..."})

@app.route('/return_to_registration', methods=['POST'])
def return_to_registration_route():
    # Diese Aktion wird vom Server behandelt, der dann ein game_update mit player_id=None sendet.
    # Die UI wird dann über den /status-Poll aktualisiert.
...
def leave_game_and_go_to_join_screen_route():
    ...
    with client_data_lock:
        client_view_data.update({
            ...
            "game_message": "Du hast das Spiel verlassen. Kehre zur Serverauswahl zurück.", 
            "error_message": None, "join_error": None,
        ...
        if "game_state" in client_view_data and client_view_data["game_state"] is not None:
            client_view_data["game_state"]["status"] = "disconnected" # Geändert zu disconnected
    ...
    if original_player_id_if_any and server_socket_global and current_socket_is_connected_for_send:
        ...
        # Wir verwenden hier nicht handle_generic_action, da der Client-Zustand bereits manipuliert wurde.
        if send_message_to_server({"action": "LEAVE_GAME_AND_GO_TO_JOIN"}):
            action_sent_successfully = True
            message_to_user = "Anfrage zum Verlassen an Server gesendet. Client zurückgesetzt."
            session.pop("nickname", None); session.pop("role_choice", None)
            with client_data_lock: client_view_data["game_message"] = message_to_user # Überschreibt vorherige Nachricht
        else:
            # send_message_to_server hat bereits eine Fehlermeldung in client_view_data gesetzt
            message_to_user = client_view_data.get("error_message", "Konnte Verlassen-Anfrage nicht an Server senden (Sendefehler). Clientseitig zurückgesetzt.")
        ...
        with client_data_lock: client_view_data["error_message"] = message_to_user # Überschreibt game_message
    ...
    # Der network_communication_thread wird den Socket schließen, da user_has_initiated_connection = False ist.

    with client_data_lock:
        response_payload = client_view_data.copy() # Sendet den *vollständig zurückgesetzten* Zustand
        response_payload["leave_request_info"] = {"sent_successfully": action_sent_successfully, "message_for_debug": message_to_user}
...
if __name__ == '__main__':
    ...
    print(f"CLIENT: Starte Flask-App auf http://0.0.0.0:{FLASK_PORT}...")
    try:
        app.run(host='0.0.0.0', port=FLASK_PORT, debug=False)
    except OSError as e:
        print(f"!!! CLIENT FLASK (FATAL): Fehler beim Starten der Flask-App auf Port {FLASK_PORT}: {e}")
        print("!!! Ist der Port bereits belegt? !!!")
