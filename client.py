...
client_view_data = {
    ...
    "early_end_requests_count": 0, # Neu für Abstimmung
    "total_active_players_for_early_end": 0, # Neu für Abstimmung
    "player_has_requested_early_end": False # Neu für Abstimmung
}
...
def network_communication_thread():
    global server_socket_global, is_connected_to_server, client_view_data
    buffer = ""
    while True:
            ...
            try:
                with client_data_lock:
                    ...
                    if not client_view_data.get("error_message") and not client_view_data.get("join_error"): 
                         client_view_data["game_state"]["status_display"] = "Verbinde mit Spielserver..."
                print("CLIENT NET: Neuer Verbindungsversuch...")
                temp_sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM); temp_sock.settimeout(5)
                temp_sock.connect((SERVER_HOST, SERVER_PORT)); temp_sock.settimeout(None)
                server_socket_global = temp_sock; is_connected_to_server = True
                print(f"CLIENT NET: Verbunden! Socket: {server_socket_global.fileno() if server_socket_global else 'N/A'}")
                buffer = "" 
                with client_data_lock:
                ...
                print(f"CLIENT NET (CONNECT TIMEOUT)")
                with client_data_lock: client_view_data["game_state"]["status_display"] = "Verbindung Zeitüberschreitung."
                time.sleep(3); continue
            except (ConnectionRefusedError, OSError) as e:
                print(f"CLIENT NET (CONNECT ERROR): {e}")
                with client_data_lock: client_view_data["game_state"]["status_display"] = f"Verbindung fehlgeschlagen."
                time.sleep(3); continue
            except Exception as e_fatal_connect: 
                print(f"CLIENT NET (FATAL CONNECT): {e_fatal_connect}")
                time.sleep(3); continue
            ...
            while '\n' in buffer:
                ...
                with client_data_lock:
                    ...
                    if msg_type == "game_update":
                        # Wenn player_id im Update null ist, bedeutet das, dass der Spieler
                        # serverseitig entfernt wurde (z.B. nach LEAVE_GAME).
                        # Dann muss der Client seine lokale player_id auch nullen.
                        if "player_id" in message and message["player_id"] is None and client_view_data["player_id"] is not None:
                            ...
                            # Weitere Resets hier, falls nötig
                        ...
                        # Alle anderen Keys normal aktualisieren
                        update_keys = [
                        ...
                        if any(crit_err in error_text for crit_err in critical_errors):
                            ...
                            is_connected_to_server = False; break 
                    
                    elif msg_type == "acknowledgement": # Z.B. nach LEAVE_GAME
                        ack_message = message.get("message", "Aktion bestätigt.")
                        client_view_data["game_message"] = ack_message
                        # Wenn die Aktion ein Verlassen war, sollte player_id schon None sein
                        # oder durch das nächste game_update genullt werden.
...
def join_game_route():
    ...
    nickname = data.get('nickname'); role_choice = data.get('role')
    if not nickname or not role_choice: return jsonify({"success": False, "message": "Name/Rolle fehlt."}), 400
    session["nickname"] = nickname; session["role_choice"] = role_choice
    with client_data_lock:
        client_view_data.update({ # Wichtige Felder für Join zurücksetzen
            "player_id": None, 
            "player_name": nickname, # Temporär bis Server bestätigt
            "role": role_choice,     # Temporär bis Server bestätigt
    ...
    response_for_js = {"success": True, "message": "Beitrittsanfrage gesendet."}
    ...
    if socket_conn_ok and is_connected_to_server:
        if not send_message_to_server({"action": "JOIN_GAME", "name": nickname, "role": role_choice}):
            response_for_js = {"success": False, "message": "Senden der Anfrage fehlgeschlagen."}
    else:
        with client_data_lock: client_view_data["join_error"] = "Nicht mit Server verbunden."
        response_for_js = {"success": False, "message": "Keine Serververbindung."}
    
    with client_data_lock: current_status = client_view_data.copy()
...
def handle_generic_action(action_name, payload_key=None, payload_value_from_request=None, requires_player_id=True):
    ...
    if requires_player_id and not player_id_for_action:
        with client_data_lock: temp_cvd = client_view_data.copy() # Sende aktuellen Zustand mit Fehler
...
def leave_game_and_go_to_join_screen_route():
    ...
    # Client-seitige Daten für den Übergang zum Join-Screen rigoros zurücksetzen
    session.pop("nickname", None) # Session-Daten löschen für frisches Formular
    session.pop("role_choice", None)

    with client_data_lock:
        client_view_data.update({
            "player_id": None, # Wichtig: ID entfernen
            "player_name": None,
            "role": None,
            "confirmed_for_lobby": False,
            "player_is_ready": False,
            "player_status": "active", # Standard für ein potenziell neues Spiel
            "current_task": None,
            "hider_leaderboard": [],
            "hider_locations": {},
            "power_ups_available": [],
            "game_message": None, 
            "error_message": None, # Alte Fehler löschen
            "join_error": None,
            "hider_location_update_imminent": False,
        ...
        if "game_state" in client_view_data and client_view_data["game_state"] is not None:
            client_view_data["game_state"]["status"] = GAME_STATE_LOBBY # Optimistisch
    ...
    with client_data_lock:
        response_payload = client_view_data.copy() # Enthält jetzt die genullten/resetteten Daten
...
if __name__ == '__main__':
    ...
    with client_data_lock: # Initial Client View Data Setup
